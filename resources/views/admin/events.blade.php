        ...
        <div id="PagesDataTable_filtersCollapse"
                        ...
                        <form method="post" action="{{url('/searchEvent')}}">
                         @csrf
                            ...
                            <div class="row" ><div class="col-md-3"><div class="form-group   "><input class="form-control filter" placeholder="Event Name" id="title" name="search" type="text"></div></div><div class="col-md-1 p-r-0"><button class="btn btn-primary filterBtn"  type="submit"><i class="fa fa-search"></i></button></div></div>
                         </form>
                        </div>
                    </div>
                 ...
                 <div class="alert alert-danger">{{$error}}</div>
                  ...
                  <div class="alert alert-info">{{session('success')}}</div>
                    ...
                    <table  class="table table-hover table-striped table-condensed dataTableBuilder" id="PagesDataTable" style="width:75%;"><thead><tr><th >Title</th><th >From</th><th >To</th><th >Time</th><th class="text-center">Action</th></tr></thead>
        ...
        <td>{{$user->title}}</td> 
               <td>                        
                @if(!empty($user->startDate))
                             <?php
                               $mystamp=strtotime($user->startDate);
                ...
                @if(!empty($user->endDate))
                             <?php
                               $mystamp=strtotime($user->endDate);
                              echo date('d-M-Y',$mystamp);  ?>
                             @else
                ...
                <td>{{$user->starttime}} - {{$user->endtime}}</td>        <td style="padding-right: 40px;">
                                                  ...
                                                  <button type="button" class="btn btn-sm btn-default dropdown-toggle" style="padding: 2px 8px 0 8px;"
                                                   ...
                                                   <form method="post" action="{{url('/editEvent')}}">
                                                   @csrf   
                                                   ...
                                                   <input type="hidden" name="id" value="{{$user->id}}" />
                                                   <li>
                                                   <button onclick="return confirm('Do you really want to delete this Event')" class="buttontext" type="submit">&nbsp;&nbsp;&nbsp;&nbsp; <i class="fa fa-trash"></i>  Delete</button>
                                                   </form>
                                                  </li>
