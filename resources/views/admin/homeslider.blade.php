    ...
    <ol class="breadcrumb">
        ...
        <section class="content">
            <div class="row">
              ...
              <form method="POST" action="{{url('/createHomeSlider')}}">
              @csrf               
...
<tbody>
   @foreach ($users as $user)
    <tr>
        ...
        <td>
          <?php 
          $say=substr($user->title,0,25);
          ?>
          @if(strlen($user->title)>25)
          {{$say}}  ...
          @else
          {{$say}}
          @endif
        </td>  
        <td><img src="resources/sliders/{{$user->url}}" alt="image" height="100" /></td>     
         <td>
              ...
              <div class="item-actions" style="padding-right: 40px;">
                  ...
                  <button type="button" class="btn btn-sm btn-default dropdown-toggle" style="padding: 2px 8px 0 8px;"
                   ...
                   <form method="post" action="{{url('/editHomeSlider')}}">
                   @csrf   
                   ...
                   <form method="post" action="{{url('/deleteHomeSlider')}}">
                   @csrf   
                   <input type="hidden" name="id" value="{{$user->id}}" />
                   <li>
                   <button onclick="return confirm('Do you really want to delete this Slider')" class="buttontext" type="submit">&nbsp;&nbsp;&nbsp;&nbsp; <i class="fa fa-trash"></i>  Delete</button>
                   </form>
                  </li>
