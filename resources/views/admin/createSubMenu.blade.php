...
<div class="box box-primary">
    ...
    <div class="box-body" style="min-height: 350px;">
            ...
            <form method="POST" action="{{url('/createSubMenu1')}}">
              @csrf
            ...
            <input class="form-control " name="url"  type="text" value="#"  required autofocus />
            </div>


            <div class="form-group required-field col-sm-6"><label for="name">Sub Menu to:</label>
             <select name="subTo" class="form-control ">
              @foreach($menus as $menu)
              @if(($menu->type=="menu")&&($menu->key1=="1"))

               <option value="{{$menu->key}}">{{$menu->key}}</option>
            ...
            <input class="form-control "  name="type" type="text" value="sub" required autofocus readonly="readonly" />
            </div>

              
            <div class="form-group  text-right" style="padding-top: 50px;"><button class="btn btn-success" type="submit"><i class="fa fa-fw fa-save"></i> Create [Sub Menu]</button>&nbsp;&nbsp;<a class="btn btn-warning" href="{{url('/menu')}}"><i class="fa fa-times"></i> Cancel</a></div>
